...
function draw(now){
  ...
  drawRadar();back.hidden=!state.drill;path.textContent=state.drill?'FUN_WIN.EXE / PROBABLE UNITS / '+state.drill:'FUN_WIN.EXE / '+(state.view==='units'?'PROBABLE UNITS':state.view==='memory'?'MEMORY MAP':'ALL FUNCTIONS');mapTitle.textContent=state.view==='memory'?'ADDRESS MAP':'FUNCTION MAP';
